...
def check():
	...
	exi = mc.listRelatives(jnt[j], typ='transform')
	if exi is None:
		station()
	else:
		for i in exi:
...
def PlusY():
	mc.select(cl=True)
	exp = mc.joint(n='{0}_expand{1}'.format(jnt[j], xyz[k+f]), rad=0.3)
	grp = mc.group(exp, n='{0}_expSpace{1}'.format(jnt[j], xyz[k+f]))
	mc.parent(grp, jnt[j])
